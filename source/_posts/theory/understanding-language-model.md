...
> 额外多说一句，上述理解仅代表个人的看法，我会在下面给出自己的解释，不一定正确，但以我目前的能力，只有这个解释能够说服我自己。（数学渣滓的悲哀 {% github_emoji cry %}）
...
假设我们有以下语料（是的，你没看错，就三句 {% github_emoji wink %}，词汇表 $V = (a, b, c)$）：
...
事实上这个神经网络模型也是一个 N-Gram 模型，即只考虑前 $n-1$ 个词的依赖关系，只不过是先将每个词都映射为连续空间中的一个词向量，再通过一个三层前馈神经网络去建模这种依赖关系（约束关系），相较于统计模型而言， 极大地增强了模型的泛化能力。模型公式如下（结合模型结构图很容易看懂了，不多 BB {% github_emoji stuck_out_tongue_winking_eye %}）：
