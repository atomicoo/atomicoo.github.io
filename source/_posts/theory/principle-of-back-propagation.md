...
我们今天的主角，**反向传播算法**（Back Propagation Algorithm，BP Algorithm），常与最优化算法（譬如梯度下降法等）结合使用的来训练神经网络，属于参数学习算法的一部分（或者说是参数学习算法的好搭档，就看你怎么理解了）。该算法会对网络中所有权重计算损失函数的梯度，并反馈给参数优化算法用以更新权重。

对于反向传播算法，有两个常见的误区在这里澄清一下：

- 反向传播算法是**梯度计算方法**，而不是参数学习算法（譬如随机梯度下降算法等）
- 理论上反向传播算法可以用于计算任何函数的梯度，而不是仅仅用于多层神经网络
...
> Ps：本文的主角以其说是反向传播算法，倒不如说是维度相容快速矩阵求导，在整个推导过程中“维度相容”贯通始终。
